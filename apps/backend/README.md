...
1. Copy the `.env.example` file:
   ...
   - `ADMIN_TOKEN` (required to access admin-only endpoints, including chat logs)
...
- `GET /api/v1/chat/logs`: Admin-only endpoint. Returns recent chat logs in reverse chronological order, with pagination and optional `success=true|false` filtering. Requires `x-admin-token` (must match `ADMIN_TOKEN`) and responds with `Cache-Control: private, no-store`.
